...
export default function Navbar(){
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink to='/' onClick={()=>{setUser(false)}}>
            ...
            {/* Mobile Menu */}
            {open && (
                <div className={`${open ? 'flex' : 'hidden'} absolute top-full z-1 left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                <NavLink to='/' onClick={()=>{setOpen(false)}}>Home</NavLink>
                <NavLink to='/allproducts' onClick={()=>{setOpen(false)}}>All Product</NavLink>
                {user && 
                <NavLink to='/allproducts' onClick={()=>{setOpen(false)}}>My Order</NavLink>
                }
